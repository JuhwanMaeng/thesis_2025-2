...
- **장기 메모리**: 중요도가 높은 사건을 FAISS 벡터 인덱스로 전환 (importance >= 0.7)
- **벡터 검색**: OpenAI 임베딩을 사용하여 의미적으로 유사한 기억을 검색 (RAG 기반)
- **대화 히스토리 통합**: 최근 대화 히스토리를 검색 쿼리와 프롬프트에 포함하여 맥락 유지
...
2. **기억 검색**: 벡터 검색을 통해 관련된 과거 기억을 검색 (episodic, persona, world 지식). 최근 대화 히스토리를 쿼리에 포함하여 맥락 유지
...
7. **중요도 평가**: 사건의 중요도를 평가하여 장기 메모리 전환 여부 결정
